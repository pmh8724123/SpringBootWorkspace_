...
public class AuthController {
	...
	public ResponseEntity<AuthResult> login(@RequestBody LoginRequest req){
		 ...
		 *  - 프론트에서는 응답상태가 404인 경우 회원가입할지, 재로그인할지 처리
		 *  - 이메일은 존재하나 비밀번호가 틀린경우 401상태(미인증) 반환 => 재로그인	
		 *  - 모두 성공 시 유저정보와 JWT토큰 반환
		 * */
		...
		try {
			...
			// 인증성공 시 accessToken, refreshToken생성하여 클라이언트의 쿠키로 전달.
	...
	public ResponseEntity<Void> logout(HttpServletRequest req){
		...
		// 로그아웃처리(쿠키 만료처리)
		ResponseCookie refresh = createTokenCookie(REFRESH_COOKIE, "", 0);
